...
function startChildBot(botId, botData) {
    try {
        ...
        const showAdminKeyboard = (chatId, liveBotData, message = "👨‍💻 *Advanced Admin Control Panel*") => {
            ...
            const adminKb = {
                keyboard: [
                    ...
                    [{ text: '💳 Reset All User Balance' }, { text: '👨‍💻 Manage Admins' }],
        ...
        bot.on('message', async (msg) => {
            ...
            // ==========================================
            // 🔐 DEVICE VERIFICATION HANDLER (WEB APP DATA)
            // ==========================================
            if (msg.web_app_data) {
                ...
                if (deviceSnap.exists() && deviceSnap.val().usedBy !== chatId) {
                    ...
                    bot.sendMessage(chatId, "⚠️ *Same Device Detected!*\nYou are allowed to use the bot, but referral bonus has been blocked for this device.", { parse_mode: "Markdown" });
                    ...
                    bot.sendMessage(chatId, "✅ Verification Successful! Welcome.");
                }
                return;
            }
            ...
            if (isAdmin) {
                ...
                // NEW: Verification Toggle Handle
                ...
                const adminCommands = {
                    ...
                    '🌐 Set Verify URL': ['awaiting_verify_url', '🌐 Send your WebApp URL for Device Verification\n(e.g., https://yourdomain.com/verify)'],
            ...
            if (childStates[stateKey]) {
                ...
                if (state.step === 'awaiting_phone') {
                    ...
                    if (state.step === 'awaiting_verify_url') updates.verifyUrl = text;
            ...
            const sendForceJoinMenu = async () => {
                ...
                // Agar Verification ON hai aur Verify URL set hai, tabhi strict verification mangenge
                if (liveBotData.verificationStatus === 'on' && liveBotData.verifyUrl) {
                    ikb.push([{ text: '✅ Verify Device & Channels', web_app: { url: liveBotData.verifyUrl } }]);
                    bot.sendMessage(chatId, "📢 Please join all required channels, then click Verify to check your device.", { reply_markup: { inline_keyboard: ikb } });
                } else {
                    // Agar OFF hai toh normal button dikhayenge
            ...
            if (text === '/start' || text.startsWith('/start ')) {
                // Referrer ka id save kar lo, paisa Verify hone par denge
        ...
        bot.on('callback_query', async (query) => {
            ...
            // Fallback Channel Verification (Jab verification OFF ho ya link na ho)
            if (data === 'check_joined_fallback') {
                ...
                if (!joined) {
                    ...
                    // Verification successful, refer reward dedo
                    ...
                    bot.sendMessage(chatId, "✅ Joined successfully! Send /start to continue.");
                }
            }
    ...
    if (masterStates[chatId] && masterStates[chatId].step === 'awaiting_token') {
        ...
        try {
            ...
            if (response.data.ok) {
                ...
                // By Default Verification band rakha hai naye bot ke liye
                const botData = {
                    ownerId: chatId.toString(), token, status: 'on', withdrawStatus: 'on', verificationStatus: 'off',
                    username: response.data.result.username, 
                    apiUrl: "", verifyUrl: "", minWithdraw: 10, maxWithdraw: 100, bonus: 5, referAmount: 20, minRefer: 0,
                    createdAt: Date.now()
                };
