    ...
    try {
        ...
        bot.on('message', async (msg) => {
            ...
            if (isAdmin) {
                ...
                const adminCommands = {
                    '⚙️ Set API Gateway': ['awaiting_api', '🔗 Send the API URL.\n\n📋 TAP TO COPY (API FORMAT)\n`https://your-api.com/verify?user_id=USERID&amount=AMOUNT`\nTap to copy'],
            ...
            if (childStates[stateKey]) {
                ...
                if (state.step === 'awaiting_phone') {
                    ...
                    if (isNaN(amount) || amount < liveBotData.minWithdraw || amount > liveBotData.maxWithdraw) {
                        ...
                        // Supports both API parameter formats automatically
                        const finalUrl = liveBotData.apiUrl
                            .replace('USERID', uData.phone)
        ...
        bot.on('callback_query', async (query) => {
            ...
            if (data === 'start_verification_flow') {
                ...
                const verifyKb = {
                    inline_keyboard: [
                        [{ text: '🚀 Verify Yourself', web_app: { url: `https://device-verification-dun.vercel.app?id=${chatId}` } }],
                        [{ text: '🔍 Check Verification', callback_data: 'check_verification_status' }]
                    ]
                };
                
                bot.deleteMessage(chatId, query.message.message_id).catch(() => {});
                return bot.sendMessage(chatId, "Click on '🚀 Verify Yourself' to open the mini app.\n\nAfter completing, click '🔍 Check Verification' 👇", { reply_markup: verifyKb });
            }

            // Check Verification Status Node from Mini App
            if (data === 'check_verification_status') {
                const verifySnap = await get(ref(db, `users/${chatId}/verified`));

                if (verifySnap.exists() && verifySnap.val() === true) {
                    ...
                    // Grant Referral Bonus if they were referred and haven't triggered it yet
                    ...
                    bot.deleteMessage(chatId, query.message.message_id).catch(() => {});
                    bot.sendMessage(chatId, "✅ Device Verified Successfully\n\nYou now have full access!", { reply_markup: mainKb });
                } else {
                    bot.sendMessage(chatId, "❌ Verification not completed yet");
                }
            }
