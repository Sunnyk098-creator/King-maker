...
// Make sure this site contains the updated HTML code provided below!
const GLOBAL_VERIFY_URL = "https://yourdomain.com/verify"; 
    ...
    try {
        ...
        bot.on('message', async (msg) => {
            ...
            if (msg.web_app_data) {
                ...
                if (deviceSnap.exists()) {
                    const record = deviceSnap.val();
                    if (record.usedBy === chatId.toString()) {
                        // User clicked verify again on the same account
                        await update(userRef, { isVerified: true });
                        bot.sendMessage(chatId, "✅ You are already verified!", { reply_markup: mainKb });
                    } else {
                        // ❌ DIFFERENT ACCOUNT TRYING TO USE SAME DEVICE IN SAME BOT
                        bot.sendMessage(chatId, "🚫 *Verification Blocked*\nThis device is already verified on another account inside this bot. Multiple accounts per device are strictly prohibited.", { parse_mode: "Markdown" });
                    }
                } else {
                    // ✅ NEW DEVICE DETECTED FOR THIS SPECIFIC BOT
                    await set(deviceRef, { usedBy: chatId.toString(), timestamp: now });
                    await update(userRef, { isVerified: true });
            ...
            if (isAdmin) {
                ...
                // ... (Bot On/Off logic remains the same)
            ...
            if (childStates[stateKey]) {
                ...
                // ... (Withdraw and phone linking remains unchanged)
                if (state.step === 'awaiting_phone') {
                    ...
                    // 🚨 ADMIN CHANNEL CHECK IMPLEMENTED HERE
                    if (state.step === 'awaiting_channel') {
                        const channelUsername = text.trim();
                        bot.sendMessage(chatId, `⏳ Checking admin status in ${channelUsername}...`);
                        
                        try {
                            const botMember = await bot.getChatMember(channelUsername, botId);
                            
                            if (['administrator', 'creator'].includes(botMember.status)) {
                                await set(ref(db, `bots/${botId}/channels/${Date.now()}`), channelUsername);
                                bot.sendMessage(chatId, `✅ Channel ${channelUsername} added successfully for force join.`);
                            } else {
                                bot.sendMessage(chatId, `❌ **Action Denied!**\nI am not an admin in ${channelUsername}.\n\n*Please make me an admin in the channel first, then try adding it again.*`, { parse_mode: "Markdown" });
                            }
                        } catch (err) {
                            bot.sendMessage(chatId, `❌ Cannot access ${channelUsername}.\nEnsure the username is correct (like @channel) and I am added as an admin there.`);
                        }
                    } 
            ...
            // (User Commands: /start, Balance, Refer, Bonus, Withdraw remain unchanged)
        ...
        bot.on('callback_query', async (query) => {
            ...
            if (data === 'start_verification_flow') {
                ...
                return bot.sendMessage(chatId, "Click on '🚀 Start Verification' below 👇", { reply_markup: verifyKb });
            }
