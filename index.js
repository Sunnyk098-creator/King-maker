...
function startChildBot(botId, botData) {
    try {
        ...
        bot.on('message', async (msg) => {
            ...
            if (!userSnap.exists()) {
                await set(userRef, { balance: 0, refers: 0, phone: "", isBanned: false, referred: false, joinedAt: now, lastAction: now });
            ...
            const sendForceJoinMenu = async () => {
                ...
                // REMOVED WEB APP VERIFICATION, ADDED SIMPLE BUTTON
                ikb.push([{ text: '✅ I have Joined', callback_data: 'check_joined' }]);
            ...
            if (text === '/start' || text.startsWith('/start ')) {
                // Process Referral if new
                if (text.startsWith('/start ') && text.split(' ')[1] !== chatId.toString() && !uData.referred) {
        ...
        bot.on('callback_query', async (query) => {
            ...
            // Verify Channels Action (Updated: No Web App needed)
